//
//  MagiMotionCoreAnimationViewContext.swift
//  MagicApp
//
...
internal class MagiMotionCoreAnimationViewContext: MagiMotionAnimatorViewContext {
    ...
    override func apply(state: MagiMotionTargetState) {
    ...
    override func startAnimations() -> TimeInterval {
        if let beginStateModifiers = targetState.beginState {
            let beginState = MagiMotionTargetState(modifiers: beginStateModifiers)
...
fileprivate extension MagiMotionCoreAnimationViewContext {
    /**
     ...
     */
    func viewState(targetState ts: MagiMotionTargetState) -> [String: Any?] {
        var ts = ts
